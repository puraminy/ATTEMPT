...
if [ "$method" = "ptat" ]; then
	...
	params="${params} --@router_temperature=1."
	#params="${params} --source_prompts=after@event@adj"
	params="${params} --@prompt_encoder_type=mlp#lstm#!emb"
	params="${params} --@num_source_prompts=5#10"
	params="${params} --@num_target_prompts=3#5"
	params="${params} --@num_prompt_tokens=3#5"
