...
while [ "$cont" -eq 1 ]; do
   ...
   case $bash_params in
      ...
      *"_atasks"*) # debug is enabled
         ...
         _task="xAttr@xIntent@xReact@oReact@oEffect@oWant@xWant@xEffect@xNeed"
         cont=1
         ;;
      ...
      *"_minus-atasks"*) # debug is enabled
         bash_params=$(echo "$bash_params" | sed "s/_minus-atasks//")
         _task="xAttr#xReact#xIntent#xWant#xEffect#xNeed"
         cont=1
         ;;
      *"_gtasks"*) # debug is enabled
         bash_params=$(echo "$bash_params" | sed "s/_gtasks//")
         _task="mrpc@stsb@qnli@qqp@rte@cola@mnli"
         cont=1
         ;;
      *"_minus-gtasks"*) # debug is enabled
         bash_params=$(echo "$bash_params" | sed "s/_minus-gtasks//")
         _task="mrpc@sst2@qnli@qqp@rte"
         cont=1
         ;;
      *"_src-gtasks"*) # debug is enabled
         bash_params=$(echo "$bash_params" | sed "s/_src-gtasks//")
         _src="src-glue-500_1_mlp_mrpc@src-glue-500_1_mlp_sst2@src-glue-500_1_mlp_qnli@src-glue-500_1_mlp_qqp@src-glue-500_1_mlp_rte"
         #glue-500_1_mlp_com
         cont=1
         ;;
      *"_sep-src-gtasks"*) # debug is enabled
         bash_params=$(echo "$bash_params" | sed "s/_sep-src-gtasks//")
         _task="mrpc#sst2#qnli#qqp#rte"
         cont=1
         ;;
...
if [ -z "$_rels" ]; then  _rels="$_task"; fi
if [ -z "$_met" ]; then
   _met="ptat"
fi
if [ "$_model" = "path" ]; then
...
if [ "$method" = "ptat" ] || [ "$method" = "adapter" ]; then
   ...
   params="${params} --@use_private_prompts=True#False"
   params="${params} --@learn_attention=True"
   ...
   params="${params} --@attn_method=rb#const"
   params="${params} --@temperature=5.#0.01"
   params="${params} --@anneal_dir=-1#0"
   ...
   params="${params} --@gen_route_methods=sigmoid@direct"
   params="${params} --route_method=direct"
   params="${params} --init_from_words=False"
   params="${params} --save_these_prompts=none"
